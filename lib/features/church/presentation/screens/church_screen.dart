...
class _ChurchScreenState extends State<ChurchScreen> {
  ...
  ChurchProfile _generateMockChurchProfile() {
    return ChurchProfile(
      ...
      aboutChurch: 'Grace Community Church is a family of believers dedicated to knowing God and making Him known. We are committed to the teachings of the Bible and fostering a community of faith, hope, and love.',
      columns: [
        ChurchColumn(id: '1', number: 101, name: 'Alpha', createdAt: DateTime.now()),
        ChurchColumn(id: '2', number: 102, name: 'Beta', createdAt: DateTime.now()),
        ChurchColumn(id: '3', number: 103, name: 'Gamma', createdAt: DateTime.now()),
        ChurchColumn(id: '4', number: 201, name: 'Delta', createdAt: DateTime.now()),
        ChurchColumn(id: '5', number: 202, name: 'Epsilon', createdAt: DateTime.now()),
        ChurchColumn(id: '6', number: 203, name: 'Zeta', createdAt: DateTime.now()),
        ChurchColumn(id: '7', number: 301, name: 'Eta', createdAt: DateTime.now()),
        ChurchColumn(id: '8', number: 302, name: 'Theta', createdAt: DateTime.now()),
      ],
      positions: [
        ...
        ChurchPosition(id: '5', name: 'Choir Member', createdAt: DateTime.now()),
        ChurchPosition(id: '6', name: 'BPMS', createdAt: DateTime.now()),
        ChurchPosition(id: '7', name: 'Choir Leader', createdAt: DateTime.now()),
        ChurchPosition(id: '8', name: 'Sunday School Teacher', createdAt: DateTime.now()),
        ChurchPosition(id: '9', name: 'Youth Leader', createdAt: DateTime.now()),
      ],
          ...
          child: Material(
            child: ColumnEditDrawer(
              ...
              onSave: (updatedColumn) {
                setState(() {
                  final index = _churchProfile.columns.indexWhere((c) => c.id == column.id);
                  if (index != -1) {
                    final updatedColumns = List<ChurchColumn>.from(_churchProfile.columns);
                    updatedColumns[index] = updatedColumn;
                    _churchProfile = _churchProfile.copyWith(columns: updatedColumns);
                  }
                });
              },
              onDelete: () {
                setState(() {
                  final updatedColumns = _churchProfile.columns.where((c) => c.id != column.id).toList();
                  _churchProfile = _churchProfile.copyWith(columns: updatedColumns);
                });
              },
              onClose: () => Navigator.of(context).pop(),
          ...
          child: Material(
            child: PositionEditDrawer(
              ...
              onSave: (updatedPosition) {
                setState(() {
                  final index = _churchProfile.positions.indexWhere((p) => p.id == position.id);
                  if (index != -1) {
                    final updatedPositions = List<ChurchPosition>.from(_churchProfile.positions);
                    updatedPositions[index] = updatedPosition;
                    _churchProfile = _churchProfile.copyWith(positions: updatedPositions);
                  }
                });
              },
              onDelete: () {
                setState(() {
                  final updatedPositions = _churchProfile.positions.where((p) => p.id != position.id).toList();
                  _churchProfile = _churchProfile.copyWith(positions: updatedPositions);
                });
              },
              onClose: () => Navigator.of(context).pop(),
  ...
  Widget _buildChurchInformationSection(ThemeData theme) {
    return ExpandableSurfaceCard(
      ...
      subtitle: 'Update the details for your church. This information may be visible to members.',
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Expanded(
                child: _buildInfoRow('Phone Number', _churchProfile.phoneNumber, theme),
              ),
          ...
          const SizedBox(height: 16),
          _buildInfoRow('About the Church', _churchProfile.aboutChurch, theme, maxLines: 3),
          const SizedBox(height: 16),
          
          // Location Information
          if (_churchProfile.latitude != null && _churchProfile.longitude != null) ...[
            Row(
              children: [
                Expanded(
                  child: _buildInfoRow('Latitude', _churchProfile.latitude!.toString(), theme),
                ),
                const SizedBox(width: 16),
                Expanded(
                  child: _buildInfoRow('Longitude', _churchProfile.longitude!.toString(), theme),
                ),
              ],
            ),
            const SizedBox(height: 16),
          ...
          if (_churchProfile.serviceSchedule != null)
            _buildInfoRow('Service Schedule', _churchProfile.serviceSchedule!, theme, maxLines: 3),
        ],
      ),
    );
  }

  Widget _buildInfoRow(String label, String value, ThemeData theme, {int maxLines = 1}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
  ...
  Widget _buildColumnManagementSection(ThemeData theme) {
    return ExpandableSurfaceCard(
      ...
      subtitle: 'Manage your church columns. Total columns: ${_churchProfile.columns.length}',
      trailing: ElevatedButton.icon(
        onPressed: () {
  ...
  Widget _buildPositionManagementSection(ThemeData theme) {
    return ExpandableSurfaceCard(
      ...
      subtitle: 'Manage member positions. Total positions: ${_churchProfile.positions.length}',
      trailing: ElevatedButton.icon(
        onPressed: () {
  ...
  List<String> _getMembersForColumn(String columnId) {
    ...
    final mockColumnMembers = {
      ...
      'col3': ['Henry Clark', 'Ivy Martinez', 'Jack Anderson', 'Kate Thompson', 'Leo Garcia'],
    };
